...
- 内置纯 JavaScript Git 引擎，可在软件中配置仓库、身份和访问令牌，无需安装系统 Git
- AI 输入框始终可编辑，运行中消息进入可撤回的引导队列，并在模型或工具步骤间主动调整当前任务
...
在“设置 → 内置 Git 同步”中选择作品，填写 HTTPS 仓库地址、同步分支、
提交者信息和访问令牌，保存后即可在写作台点击“同步”。软件会自动提交本地变化，
拉取并合并云端更新，再推送到远程仓库。访问令牌使用 Electron 系统安全存储加密，
不会写入作品目录或远程地址。

发生内容冲突时，同步会暂停并列出冲突文件。编辑并保存这些文件、删除冲突标记后，
回到提示框点击“已处理，继续同步”，软件会在内部完成合并提交和推送。
